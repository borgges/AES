...
const BotName = 'AES bot'; 
const instagram = 'http://www.instagram.com/'; 
const aktif = '08:00 - 23:59';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:meu criador\n Borgges' // criador
            + 'ORG: AlianceSport;\n AES bot' // nome do bot
            + 'TEL;type=CELL;type=VOICE;waid=Confidencial: confidencial\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '!'
blocked = []
limitawal = '999999999'
...
async function starts() {
    ...
    •            @${num.split('@')[0]}was E j e c t e d
                      1 impostor restante   。　.
    　 　　。　　 　　　　ﾟ　　　.　      　　　.
    ,　　𝐀𝐯𝐢𝐬𝐨 𝐩𝐚𝐫𝐚 𝐯𝐨𝐜𝐞 𝐪𝐮𝐞 𝐬𝐚𝐢𝐮 @${num.split('@')[0]} 👋👋👋 👋`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
      		...
      		mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO AES Bot*',
					benned: 'Você para a banido do bot, por favor, contate o proprietário para retirar seu ban',
					ownerG: '[❗] Skiller? Este é um recurso especial para o Skiller❌',
					ownerB: '[❗] Skiller? Este é um recurso especial para o Skiller ❌',
					userB: `──「 LISTA de NICKNAMES 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado na lista de integrantes do AES. Registre-se para fazer amizade no joguinho de fada por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar seunome|17\n\n──「 *𝐀𝐄𝐒 𝐁𝐎𝐓* 」──`,
			...
			const ownerNumber = ["confidencial"] // coloque seu numero
			const mod = [ownerNumber,"confidencial"]//seu numero
			const adminbotnumber = ["confidencial"]//seu numero
			const frendsowner = ["confidencial"]//seu numero
			const premium = ["Fala com o admin.",] 
...
reply("oi posso ajudar")
	...
	if (messagesC.includes("!faustinho")){
			...
			switch(command) {
				...
				case 'dellvip':
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o proprietário ou digite *!Daftarvip um* para adquirir o acesso Premium!' ,text, { quoted: mek })
...
case 'closegc':
				...
				//_FIGURINHAS/STICKER
case 'take':
                    var namaPackss = AES bot
                    var authorPackss = BorggesG3X
...
case 'premiumlist':
               ...
               if (!isPremium) return reply(mess.only.premium)
					...
					reply(mess.wait)
					client.sendMessage(from, 'Adicionando nickname à lista com sucesso' , text, { quoted: mek })
...
case 'antilink':
				...
				//INFO BOT
case 'owner':
                    client.sendMessage(from, {displayname: "BORGGES", vcard: vcard}, MessageType.contact, { quoted: mek})
...
case 'daftarvip': 
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Borgges\n*INSTA:* @eu_borgges\n\nEspero que tenham gostado do bot 🐊🚩️'})
					break
				//_IMAGENS
				case 'boanoite':
					memein = await kagApi.memeindo()
